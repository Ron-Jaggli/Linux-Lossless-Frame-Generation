...
void Capture::handleProcess() {
    ...
    if (!corrupted && width_ > 0 && pool_created_) {
        ...
        if (ok) {
            ...
            // The probe verdict is needed at publish time so the pool can
            // track unique frames for pair leases. Without a probe result
            // the frame counts as unique; cadence stays unlocked then, so
            // nothing interpolates from it.
