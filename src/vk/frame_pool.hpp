...
// Triple-buffered pool of GPU images that decouples the capture thread
// (writer) from the render thread (reader). Published images are always in
// TRANSFER_SRC_OPTIMAL layout. Later milestones hang duplicate detection and
// frame interpolation off this same pool.
class FramePool {
public:
    struct ReadLease {
        int index = -1;
    ...
    // Writer side (capture thread). acquireWrite never blocks: with three
    // slots there is always one that is neither published nor being read.
    int acquireWrite();
    VkImage image(int index) const { return slots_[index].image; }
    void publish(int index, uint64_t seq, double t_capture);
